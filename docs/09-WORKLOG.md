...
| Status | Home built section by section; method and motion bar are D-038. Sections 1 and 2 approved by Mateo on the real page, committed as `c42df25` and `f33731a` on branch `wu-04-home`, and **pushed** to `origin/wu-04-home` on 2026-09-12. No PR opened yet, so CI has not run on this work: `.github/workflows/ci.yml` only triggers on push to `main` and on `pull_request`; the same gate was run locally instead (`pnpm build` green on both apps). Pending from the original WU-04 plan: DripCard, Carousel, CtaBand, to be reconciled with this build. |
| Last chat | 2026-09-11 / 12 · WU-04 (Home sections 1 and 2, committed and pushed) |
| Waiting on Mateo | Say whether to open the PR into `main` (that is what runs CI) · Rotate the `sanalys_owner` password (open since 2026-09-09) · Move the age private key off the Desktop into the password manager |
...
### 2026-09-12 · WU-04 · Branch pushed, local build in place of CI
- **Asked for:** "push".
- **Done:** `git push -u origin wu-04-home` — new remote branch, tracking set, two commits (`c42df25`, `f33731a`).
- **CI did not run:** `.github/workflows/ci.yml` triggers on `push` to `main` and on `pull_request` only, so a work-unit branch on its own starts nothing. The same gate was run locally instead: `SKIP_ENV_VALIDATION=1 pnpm build`, green for `web` and `sistema` (both prerender `/` and `/_not-found` as static), on top of `pnpm lint`, `pnpm --filter web typecheck` and `pnpm test` already green before the commit.
- **Left uncommitted on purpose:** `.claude/launch.json` (dev-server config for the preview pane: web on 3000, sistema on 3001) and `web/next-env.d.ts` (regenerated by `next typegen`). Mateo asked for a push, not for these to be added.
- **Found:** a Bash heredoc swallowed one backslash in a Python path (`docs 9-...` became ` ` = null char). Write these scripts with forward slashes.
- **Next:** Mateo says whether to open the PR into `main`; then Home section 3 on verde.

### 2026-09-12 · WU-04 · PR #1 opened, CI green, line endings fixed
- **Asked for:** "si a todo" — open the PR and commit the two pending files.
- **Done:** `.claude/launch.json` and the push record committed (`5dbd604`), branch pushed, **PR #1** opened into `main` (https://github.com/MateoGaviraghi/sanalys/pull/1) with the sections, what is not included and the checks in the body.
- **CI:** green on the PR — install with the frozen lockfile, typecheck, lint, test and build, the first CI this work has seen (a work-unit branch alone does not trigger it).
- **Found and fixed:** `docs/09-WORKLOG.md` had landed in the repo with CRLF because a Python helper wrote it in Windows text mode (G-064); normalized back to LF in `bdf4432`, and no CRLF file remains.
- **Next:** Mateo says what comes next; the PR is open and mergeable, not merged.
