...
### WU-04 · Design · R0 (no code merged; design constraints written)
Files: `docs/05-DISENO.md` (amend), `web/design/` (reference sheet: one HTML playground per component family, not shipped).
Done: the three stance decisions applied to every page as a component-level spec; carousel behaviour; motion inventory confirmed against the budget; 3D verdict re-checked; placeholder photo set chosen and duotoned; client approves look and structure in writing (D-010). Skills to load: `web-distintiva`, `ui-ideas`, `motion-web-senior`; libraries `animate`, `find-animation-opportunities`, `tailwindcss-mobile-first`.
